                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-5 gap-3">
                                @foreach (range(1, 5) as $i)
                                    @php
                                        $isSelected = $userFeedback && $userFeedback->instructor_understanding == $i;
                                        $isMuted = $readOnly && !$isSelected;
                                    @endphp
                                    <label
                                        class="cursor-pointer group text-center {{ $isMuted ? 'opacity-40 grayscale scale-95' : '' }}">
                                        <input type="radio" name="instructor_understanding" value="{{ $i }}"
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-5 gap-3">
                                @php
                                    $options = [
                                        ...
                                        ['label' => 'Probably No', 'val' => 'Probably No', 'icon' => 'bi-emoji-frown'],
                                        [
                                            'label' => 'Definitely No',
                                            'val' => 'Definitely No',
                                ...
                                @foreach ($options as $opt)
                                    @php
                                        $isSelected = $userFeedback && $userFeedback->room_suitability == $opt['val'];
                                        $isMuted = $readOnly && !$isSelected;
                                    @endphp
                    ...
                    <div class="space-y-12">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-5 md:grid-cols-10 gap-2">
                                @foreach (range(1, 10) as $i)
                                    @php
                                        $isSelected = $userFeedback && $userFeedback->attendance_system_rating == $i;
                                        $isMuted = $readOnly && !$isSelected;
                                    @endphp
                                    <label class="cursor-pointer {{ $isMuted ? 'opacity-30 scale-90' : '' }}">
                                        <input type="radio" name="attendance_system_rating" value="{{ $i }}"
                                            class="peer sr-only" required {{ $isSelected ? 'checked' : '' }}>
                                        <div
                                            class="aspect-square flex items-center justify-center rounded-xl border text-xs font-bold transition-all
                                            {{ $readOnly
